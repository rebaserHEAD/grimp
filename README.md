...
- **Desktop app**: native menus, native open/save dialogs, and local fork loading.
  Packaged as a Windows portable `.exe` and a Linux `.AppImage`. This is how you run GRIMP.
- The same app also runs in a browser during development (`npm run dev`), pointed at a
...
code is MIT-licensed and most assets are CC-BY-SA 3.0. GRIMP reads a fork's content live
from disk and does not redistribute it; if you ever bundle a fork's content into a build,
