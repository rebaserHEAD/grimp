...
export const App: React.FC = () => {
  ...
  menuCommandRef.current = (command: string) => {
    switch (command) {
      ...
      case 'view:showGrid':
        ...
        markSceneDirty();
        break;
      case 'view:showEntities':
        setShowEntities((e) => !e);
        markSceneDirty();
        break;
      case 'view:showSpaceBackground':
        setShowSpaceBackground((b) => !b);
        markSceneDirty();
        break;
  ...
  const requestPrompt = useCallback(
    (opts: { title: string; defaultValue?: string; onSubmit: (value: string) => void }) => {
  ...
  const rotateSelectedDecals = useCallback(
    (delta: number) => {
      const activeGrid = state.grids[state.activeGridIndex];
  ...
  const handleUpdateEntity = useCallback(
    (updated: import('./import/mapImporter').ImportedEntity) => {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      const camera = cameraRef.current;
      const tile = camera.screenToTile(e.clientX - rect.left, e.clientY - rect.top, rect.width, rect.height);
      setCursorTile({
        x: Math.floor(tile.x),
        y: Math.floor(tile.y),
      });
  ...
  return (
    <div className="flex flex-col w-full h-full">
      {showDisclaimer && (
        ...
        >
          ...
          >
            ...
            />
            <h2 style={{ color: '#fff', margin: '0 0 16px', fontSize: 20 }}>Early Development</h2>
            <p style={{ margin: '0 0 12px' }}>
              This map editor is under <strong style={{ color: '#fff' }}>active development</strong>. Expect bugs,
        ...
        (() => {
          ...
          return (
            <MapPropertiesModal
              ...
              tileCount={tileCount}
              onSetIdentity={(name, desc) => dispatch({ type: 'SET_GRID_IDENTITY', gridUid: propsGridUid, name, desc })}
        ...
        <div className="flex flex-col min-w-[280px] max-w-[400px] w-[20vw] bg-panel border-l border-subtle overflow-hidden">
          ...
          {(state.activeTool === 'cableDraw' || state.activeTool === 'pipeDraw') && (
            <CollapsiblePanel title="Infrastructure" defaultOpen={true}>
              <InfrastructurePanel selection={infraSelection} onChange={handleInfraChange} />
            </CollapsiblePanel>
          )}
      ...
      <StatusBar state={state} cursorTileX={cursorTile.x} cursorTileY={cursorTile.y} statusMessage={statusMessage} />
    </div>
  );
};
